...
+ Response
    ...
    > insStdt : 보험시작일 -> 계약일자 + 1일 (시간은 00:00:00 셋팅)  
    > insEnddt : 보험종료일 -> 보험시작일 * 계약기간 (시간은 23:59:59 셋팅)  
    > totInsPrem : 총보험료 -> 담보테이블 가입금액/기분금액 값에 계약기간을 곱한 값  
...
+ Request  
    ...
    #Request 파라미터  
    > contInfo : 계약정보 -> 계약테이블에 컬럼 중 어떤 값을 입력해도 결과가 조회 된다. 
...
+ Response
    ...
    ```
    #Response 설명
    > CONT_MST 테이블에 저장된 값이 LIST로 출력되며 각 항목별 상세 정의는 *계약 생성 API* 와 동일
...
+ Response
    ...
    ```
    #Response 설명
    > CONT_MST 테이블에 청약상태만 변경됨. 변경된 데이터를 리턴함.
